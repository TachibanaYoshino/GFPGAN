...
def argsparse():
    ...
    parser.add_argument('-i', '--input',type=str,default='../inputs/videos', help='Input image or folder. Default: inputs/whole_imgs')
    ...
    parser.add_argument('-sr', '--realesrgan', choices=['', 'realesrgan'], default="realesrgan", help='background upsampler. Default: realesrgan')
...
if __name__ == '__main__':
    ...
    video_out = cv2.VideoWriter(os.path.join(args.output,os.path.basename(args.input)), cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (width*upscale, height*upscale))
    while True:
        # print("get me")
        ret, frame = vid.read()
        if not ret:
